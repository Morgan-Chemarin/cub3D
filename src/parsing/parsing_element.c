...
int	parse_texture(t_map *map_data, char **split)
{
	...
	if (*texture != NULL)
	{
		ft_putstr_fd("Error: Duplicate texture identifier\n", 2);
		return (0);
	}
...
int	get_rgb_values(char *rgb_definition, t_color *color)
{
	...
	{
		ft_putstr_fd("Error: RGB values must be in range [0-255]\n", 2);
		return (0);
	}
	color->r = r;
	color->g = g;
...
int	parse_color(t_map *map_data, char **split)
{
	...
	if (color->r != -1)
	{
		ft_putstr_fd("Error: Duplicate color identifier\n", 2);
		return (0);
	}
...
int	check_element(char *line, t_map *map_data)
{
	...
	{
		ft_putstr_fd("Error: Invalid identifier in file:'", 2);
