...
int	get_texture_x(t_data *data, t_ray *ray, double c_dist, t_img *texture)
{
	int		texture_x;
	double	wall_x;

	if (ray->axis_orientation == 0)
		wall_x = data->player.y + c_dist * ray->ray_dir_y;
	else
...
void	draw_wall(double corrected_dist, t_data *data, t_ray *ray, int x)
{
	...
	wall_height = (int)(data->screen_h / corrected_dist);
	...
	wall_height = get_texture_x(data, ray, corrected_dist, texture);
